...
// Gán nhãn cụm (giả lập từ K-means)
int clusterLabels[MAX_CUSTOMERS + 1] = {0, 2, 1, 0, 1, 2, 2, 0, 1, 1, 0};

// Hàm in thông tin giao dịch
...
int main() {
    ...
    for (int i = 0; i < numTransactions; i++) {
        ...
        // Đếm giao dịch vào cuối tuần (Thứ 6 - 5, Thứ 7 - 6)
    ...
    bool printed[MAX_CUSTOMERS + 1] = {false};

    for (int i = 0; i < numTransactions; i++) {
        int id = transactions[i].customerID;

        if (offer[id] && !printed[id]) {
            printed[id] = true;

            printf("Khach hang #%d [Cum %d] - Tong chi tieu: %.2f VND\n", id, clusterLabels[id], spending[id]);

            for (int j = 0; j < numTransactions; j++) {
                if (transactions[j].customerID == id) {
                    printTransactionDetails(&transactions[j]);
                    printf("-------------------------------\n");
                }
            }
